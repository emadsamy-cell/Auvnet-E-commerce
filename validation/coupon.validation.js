...
exports.createCoupon = {
  body: joi
    .object({
      ...
      discountType: joi
        .string()
        .valid(discountType.PURCHASE, discountType.SHIPMENT)
        .required()
        .messages({
      ...
      couponUsage: joi
        .object({
          type: joi
            .string()
            .valid(usageLimit.UNLIMITED, usageLimit.LIMITED)
            .required()
            .messages({
          ...
          count: joi.alternatives().conditional("type", {
            ...
            otherwise: joi.forbidden().messages({
              "any.unknown":
                "Usage count for coupon cannot be provided for UNLIMITED usage type.",
            }),
          }),
        })
        .required()
      ...
      userUsage: joi
        .object({
          ...
          count: joi.alternatives().conditional("type", {
            ...
            otherwise: joi.forbidden().messages({
              "any.unknown":
                "Usage count for user cannot be provided for UNLIMITED usage type.",
            }),
          }),
        })
        .required()
      ...
      termsAndConditions: joi.object({
        minPurchaseValue: joi.number().min(0).optional().messages({
          "number.min":
            "Minimum purchase value must be greater than or equal to 0.",
          "number.base": "Minimum purchase value must be a number.",
        }),
        maxDiscountLimit: joi.number().min(0).optional().messages({
          "number.min":
            "Maximum discount limit must be greater than or equal to 0.",
          "number.base": "Maximum discount limit must be a number.",
        }),
        audienceLocation: joi
          .object({
            type: joi
              .string()
              .required()
              .valid(
                audienceLocation.REGION,
                audienceLocation.COUNTRY,
                audienceLocation.CITY
              )
              .messages({
                "any.only": `Audience location type must be ${audienceLocation.REGION}, ${audienceLocation.COUNTRY}, or ${audienceLocation.CITY}.`,
                "any.required":
                  "Type is required when audienceLocation is provided.",
              }),
            location: joi.string().required().messages({
              "any.required":
                "Location is required when audienceLocation is provided.",
      ...
      products: joi
        ...
        .items(
          joi.string().length(24).messages({
            "string.length":
              "Each product ID must be exactly 24 characters long.",
        ...
        .messages({
          ...
          "array.includes":
            "All items in the products array must be valid product IDs.",
        }),
      ...
      categories: joi
        ...
        .items(
          joi.string().length(24).messages({
            "string.length":
              "Each category ID must be exactly 24 characters long.",
        ...
        .messages({
          ...
          "array.includes":
            "All items in the categories array must be valid categories IDs.",
        }),
    })
    .xor("discountValue", "discountPercent")
    .messages({
      "object.xor":
        "Either discount value or discount percent must be provided.",
    }),
};
...
exports.getCoupons = {
  query: joi.object({
    status: joi
      .string()
      .valid(status.ACTIVE, status.EXPIRED)
      .optional()
      .messages({
...
exports.updateCoupon = {
  body: joi
    .object({
      couponType: joi
        .string()
        .valid(couponType.GLOBAL, couponType.TARGET)
        .optional()
        .messages({
    ...
    .nand("discountValue", "discountPercent")
    .messages({
      "object.nand":
        "Either discount value or discount percent can be provided.",
    }),
  params: joi.object({
    id: joi.string().length(24).required().messages({
      "string.length": "Coupon ID must be exactly 24 characters long.",
